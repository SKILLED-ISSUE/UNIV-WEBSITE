<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Exhibits</title>
  <!--<link rel="stylesheet" href="../navigation/navigation.css" />-->
  <link rel="stylesheet" href="../assets/css/main-style.css" />
  <link rel="stylesheet" href="../navigation/about-style.css" />
  <link rel="icon" href="../assets/PUPSJ-LOGO.png" type="image/icon type" />
</head>

<body>
  <header class="header-container">
    <?php include('../assets/static-html/header.html'); ?>
  </header>
  <section class="area program">
    <div class="header">
      <img id="header-img" src="../assets/abouts/1.jpg" />
      <div class="header-text" class="fade-in">

        <h id="header2" class="fade-in">Exhibits</h>
      </div>
    </div>
    <div class="exhibits-card">
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/12oeVeO4vng_6ZL0101BKQlky1I2D3vuM/view" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/ovpsas-manual-of-operations.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/1AJ3CH_scPuT0tuXeveiVgRCPc0gdUv9g/view" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/pupsj-4-year-plan.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/0B1BuDAuN0r8SbmFEU1RJUDFYU1E/view?resourcekey=0-ntyYkBAnyIFKdRJWhiKEcw" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/citizens-charter.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/0B1BuDAuN0r8SX1BWX2NSN3FURzg/view?resourcekey=0-oi8lUy9PCFysh0FDyL5ipw" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/student-handbook.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/1xzD2TuHdmky2ZtDvzr2ZsWBwLohHU4HG/view" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/university-code.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/15PagJY66WllFerrmPlFWVRnRgcFkAVdL/view?usp=drive_link" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/bshm-cmo.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/1SwXEcjaXrSeeexgA4nru6UYeWMllRRDl/view?usp=drive_link" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/bsit-cmo.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a class="link-exhibits" onclick="openSyllabiDialog()">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/syllabi.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a class="link-exhibits" onclick="openIMDialog()">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/instructional-materials.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/1hmTc7tNBhUhng7rm_Vnfh-WTd0JhUOOJ/view" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/ambisyon-natin-2024.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://pdp.neda.gov.ph/philippine-development-plan-2023-2028/" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/updated-ph-dev-plan.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/1rRGCWrMKD74uAoI-K34wkzR9yLUtQEVG/view" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/un-sdg-2030.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://www.dost.gov.ph/knowledge-resources/downloads/file/5669-harmonized-national-r-d-agenda-2022-2028.html" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/harmonized-natl-research-adn-devt-agenda.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/1rxqdcpYLd2senFC_mVXVAnFJklRX4pEu/view" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/cmo-52-s-2016.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a class="link-exhibits" onclick="openReferencesDialog()">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/university-research-national-references.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a href="https://drive.google.com/file/d/15LS2dxw6CslL0fZvIBadNSZcHwGXsUlb/view" target="_blank" class="link-exhibits">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/cmo-no-9-s-2013.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a class="link-exhibits" onclick="openPoliciesDialog()">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/university-research-related-policies.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a class="link-exhibits" onclick="openDevelopmentDialog()">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/university-research-and-development-manual.png" />
        </a>
      </ul>
      <ul class="ul-exhibits">
        <a class="link-exhibits" onclick="openDisertationDialog()">
          <img id="area-image" class="fade-in" src="../assets/Exhibits/university-thesis-and-disertation.png" />
        </a>
      </ul>
    </div>
  </section>

  <section id="syllabi-dialog" class="exhibits-dialog-overlay">
    <section class="exhibits-dialog-box">
      <section class="header-dialog">
        <button class="close-btn" onclick="closeSyllabiDialog()">
          <!--<img src="../assets/close.png" />-->
          󰌑
        </button>
        <div class="title-dialog">Syllabi</div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">BSHM</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1E_XwftejCwxywH31yaPYFDXYRIFkUDD9/view?usp=drive_link" target="_blank">
                Kitchen Essentials and Basic Food Preparations (Lab)
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/17cyju06O1WOT7UNKIwsw2tqw-t2NK3Tb/view?usp=drive_link" target="_blank">
                Kitchen Essentials and Basic Food Preparations (Lecture)
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1h7kyFkbb9_d0IwWzgK_wOez6IeboPm7H/view?usp=drive_link" target="_blank">
                Fundamentals in Food Service Operations
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1O_iuntPQgCTyywgkZMsjGdJcdN_ssuJo/view?usp=drive_link" target="_blank">
                Fundamental in Lodging Operations
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1amvb1_o5WHCcuyWdLmZECsqeS2ZssZgA/view?usp=drive_link" target="_blank">
                Revenue Management
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1Ek3izu-x6W078U7nuuPKyB0oOXtsssE2/view?usp=drive_link" target="_blank">
                Recreation and Leisure Management
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1RXWnqovTPdIdBW2-cOA7AYIwJU_lcd31/view?usp=drive_link" target="_blank">
                Risk Management as Applied to Safety, Security and Sanitation
              </a>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">BSIT</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1cSHvj12VzEsR613Nnby7zX4aFAJ3JG87/view?usp=drive_link" target="_blank">
                Multimedia
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1SQdvy2gRb_w7WIU1zshTqsYu5JZMaPrc/view?usp=drive_link" target="_blank">
                Intro to Computing
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1sMXO-q76rvIXtmrEaAAo7uaMYkUdsJjT/view?usp=drive_link" target="_blank">
                Discrete Structures
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1GyB2pFIurbYor7uF2sSDtXq9O53F6UTh/view?usp=drive_link" target="_blank">
                Data Communications and Networking
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/17rorssAXEmillWqMm_BKvxyCpqo8Joo1/view?usp=drive_link" target="_blank">
                OOP
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1EYxdPz-0-WqbkhiJ7Ixn7mwIdjlU6SaR/view?usp=drive_link" target="_blank">
                Programming 1
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1qHEHB9DltGxoA6B1cvy0aUMQaAq3No5c/view?usp=drive_link" target="_blank">
                Programming 2
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1r4JPb3eYPgOBjTXf7oT0rjsl1HkMjkZ7/view?usp=drive_link" target="_blank">
                Programming 3
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1SJANauEQtclCzDUnJIXaLjZn7owYlHpd/view?usp=drive_link" target="_blank">
                Practicum
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1hlqWRrjOcmeiqG53e6JkSI2KBGv-UuMi/view?usp=drive_link" target="_blank">
                Integrative Programming
              </a>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">General Education</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1BWyIht2dhX9GfLyzUYu5OarQBge-Woqz/view?usp=drive_link" target="_blank">
                Filipinolohiya
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1lfuHhJnmIzyLrNYUdVxf9cKtGHkSakBR/view?usp=drive_link" target="_blank">
                Panitikang Filipino
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/14IzUjR_kV1EIPtb5lksFeKhwKu1og1QA/view?usp=drive_link" target="_blank">
                Buhay at Mga Gawa ni Rizal
              </a>
            </li>
          </ul>
        </div>
      </section>
    </section>
  </section>

  <section id="IM-dialog" class="exhibits-dialog-overlay">
    <section class="exhibits-dialog-box">
      <section class="header-dialog">
        <button class="close-btn" onclick="closeIMDialog()">
          <!--<img src="../assets/close.png" />-->
          󰌑
        </button>
        <div class="title-dialog">Instructional Materials</div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">BSHM</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1PZRDrCaOsMLNWUbGHRw4OMbJlo_svpf6/view?usp=drive_link" target="_blank">
                Bar and Beverage Management
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/118rdZaUXtsm7MkNWTqSqjERo4u-bd814/view?usp=drive_link" target="_blank">
                Bread and Pastry
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1U7RgUoJ8WTe2xtUJJU0CAjJfWuAzK_aw/view?usp=drive_link" target="_blank">
                Ergonomics
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1nZACOWSKde6R-MJ9tHt3PtZK7iE_ma4F/view?usp=drive_link" target="_blank">
                Fundamentals in Lodging Operations
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1e3ZS7nzC0CenJ_3fFop-0JPtoSkyhcdm/view?usp=drive_link" target="_blank">
                Applied Business Tools and Technologies
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1CycyPWiAV39jvk_ETPfYsVg9hXYTB0i5/view?usp=drive_link" target="_blank">
                Introduction to MICE
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/15O5J2zHT62cPCHhmu9WXE_9Mj9N42TTs/view?usp=drive_link" target="_blank">
                Tourism and Hospitality Marketing
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1XhIykO78Z259d-bVPJKb7tesZOJV7nfF/view?usp=drive_link" target="_blank">
                Crowd and Crisis Management
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1ZaaH3Y7pFGSUvoskfvk1GX-rFNHfqBmR/view?usp=drive_link" target="_blank">
                Rooms Division
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1cVmD14Tsp2PaEVP6G3PO7UwTNq0do5mW/view?usp=drive_link" target="_blank">
                Kitchen Essentials
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1C1P1wa79D4MZ3TJYPznkSekdS3OmyZXr/view?usp=drive_link" target="_blank">
                FOLA 2
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1ZOoEXU9RuG_ObzIj7gaQ4k6WErn0kDAG/view?usp=drive_link" target="_blank">
                Applied Business Tools and Technologies
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1sTbkvAOzhGTnucLjD7J6kH3SpXFrv-3K/view?usp=drive_link" target="_blank">
                Revenue Management
              </a>
            </li>
            <li class="list-header">
              <strong>SCM</strong>
              <ul>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1HalHb1CmcV48sLrQXopJTIbT9e5IVa3C/view?usp=drive_link" target="_blank">
                    Topic 3 TSC Coordination
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1s0ssLDS1qOqyeM7wBf8YKKDusJtiYTy3/view?usp=drive_link" target="_blank">
                    Topic 5 TSC Competition
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1vbFUhe3KaILSgOpNSrF_ckf9JKa13Sh8/view?usp=drive_link" target="_blank">
                    Topic 6 Chain Vs Chain Competition
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1GW1XE6ldshcESR28_6p9oY1InMiAruNn/view?usp=drive_link" target="_blank">
                    Topic 7 Tourism Distriution Channel
                  </a>
                </li>
              </ul>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">BSIT</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1b6tm5JgkyKt-3SBEi47WWUzG6WtBTIPe/view?usp=drive_link" target="_blank">
                Basic Networking
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1EE-Rr7l9k9TgDNdsQqCRar17d6T8Bsfd/view?usp=drive_link" target="_blank">
                Business Intelligence
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1ZnjdPPnrl7mBtnUpqJwtaRtZn9nZ3NtH/view?usp=drive_link" target="_blank">
                CyberSecurity Fundamentals
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1GvHlVKiuJD9FZIdsdl941GSIYTXmFBFo/view?usp=drive_link" target="_blank">
                Modeling and Simulation
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/10d4gIATER8QsHJycyU28rUktYRmLnnA0/view?usp=drive_link" target="_blank">
                COMP-009-OOP
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1x9kmKOH1dr47jgQ_9Sq4ahdQzrzlf2XU/view?usp=drive_link" target="_blank">
                Programming 2
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1Pcd-P9DFRaqacN8ESCgwudMhg9bRuRWq/view?usp=drive_link" target="_blank">
                Operating Systems
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1kVrYY9tf0vZFkZMaOZkgEilZlULBFekx/view?usp=drive_link" target="_blank">
                Data Mining and Warehousing-converted
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1qfpjB_JqCz7PD5ZFf4USuQpooJikhcnH/view?usp=drive_link" target="_blank">
                Capstone 1
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/17qKiyV_V0lnMBOEcKV4Vl5I-n0u7-7DU/view?usp=drive_link" target="_blank">
                Capstone 2
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1gQD3C_MiCgcCytBDBhrAQkKXpZZecxFr/view?usp=drive_link" target="_blank">
                Programming 3-COBOL
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1f04Ml3q6Fa01CuU5pvVEj0rXB4wquBay/view?usp=drive_link" target="_blank">
                SAD
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1m7CmEudubd4mJ6CiS40aDn3F-ax8Oo7w/view?usp=drive_link" target="_blank">
                Integrative with data analytics
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1AYzCqXOv1SPtFVYgoHJc9hQDeoDiS-Fs/view?usp=drive_link" target="_blank">
                Computer Programming 1
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1A81-xRhSxV6x_Ty96JkVO7OfCHZpGV9q/view?usp=drive_link" target="_blank">
                Multimedia
              </a>
            </li>
            <li class="list-header">
              <strong>General Education</strong>
              <ul>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/165JTIHvLTzCYgolGlaLPot8HP03_MENs/view?usp=drive_link" target="_blank">
                    Readings in Philippine History
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/13bXJo_d2T31_4m2RqNlEEIqUK6NDU1a7/view?usp=drive_link" target="_blank">
                    Art Appreciation
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1H9tJtRunIqKKMLfLi_fEAx5LheVxUTzY/view?usp=drive_link" target="_blank">
                    Politics and Governance
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1IsCUt-S67D5iwU-MYwoBhdsnKyMPAGbn/view?usp=drive_link" target="_blank">
                    Contemporary World
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/13q4xwzV0-CUDC_p8xTAhyC-AxTxHLToW/view?usp=drive_link" target="_blank">
                    Mathematics in the Modern World
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/11MSXx3want0y1U3AKg0IoggR_9vVbICM/view?usp=drive_link" target="_blank">
                    Ethics
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/15yFNex3tXLCcpJinJoaHUZlJMk0CJfSa/view?usp=drive_link" target="_blank">
                    Panitikan ng Pilipinas
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1HNW2YngWKRZLvJdc5Dfl5yWphD7MHJ4F/view?usp=drive_link" target="_blank">
                    Science, Technology and Society
                  </a>
                </li>
                <li class="sip">
                  <a href="https://drive.google.com/file/d/1s3v8eMnOsD-_jMODsKf5f7co9uTMGItV/view?usp=drive_link" target="_blank">
                    Understanding the Self
                  </a>
                </li>
              </ul>
            </li>
          </ul>
        </div>
      </section>
    </section>
  </section>

  <section id="references-dialog" class="exhibits-dialog-overlay">
    <section class="exhibits-dialog-box">
      <section class="header-dialog">
        <button class="close-btn" onclick="closeReferencesDialog()">
          <!--<img src="../assets/close.png" />-->
          󰌑
        </button>
        <div class="title-dialog">University Research National References</div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">References</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1vOhe4mv6J97cOamOBr9NINqlOMiJX1SH/view?usp=drive_link" target="_blank">
                CHED NHERA-2
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1HhyP2ohVJdp0yejOeaZGqdcJJICeOUkO/view?usp=drive_link" target="_blank">
                DOST Approved Harmonized National RD Agenda 2017-2022
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1n2ItuxwiwgvSh_a709fD6hwBseWu1goN/view?usp=drive_link" target="_blank">
                DOST HNRDA 2022-2028
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1s-rFJvY7cLfALOujkIJ7QZNqYE-pUJxD/view?usp=drive_link" target="_blank">
                Dr. Ceocaris CHED Acad Industry PAGE April 13
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1CyT3Sfosfv81UnpTXAFZZcKbcZYisNgL/view?usp=drive_link" target="_blank">
                NEDA PDP 2017-2022
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1eOxPZLVdXCOzX9M2jLDU5VO1Cxi79SLl/view?usp=drive_link" target="_blank">
                NEDA PDP 2023-2028
              </a>
            </li>
          </ul>
        </div>
      </section>
    </section>
  </section>

  <section id="policies-dialog" class="exhibits-dialog-overlay">
    <section class="exhibits-dialog-box">
      <section class="header-dialog">
        <button class="close-btn" onclick="closePoliciesDialog()">
          <!--<img src="../assets/close.png" />-->
          󰌑
        </button>
        <div class="title-dialog">University Research Related <br> Policies</div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Research Related Policies</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1MMtPhzYzbgIDArQrw_QzPvIJxP7PxxJP/view?usp=drive_link" target="_blank">
                EO 9 s2022 Revised Guidelines on ReAP for Regular Full-Time Faculty Members
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1od-NU-WSyUMxmJBfEQZD9u3Xx_Td2AhP/view?usp=drive_link" target="_blank">
                EO 15 s2023 Annex Technology Transfer & Commercialization Prcess
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/13ngZhvhrFG9N9etAcJkss8KzTjEyoQbW/view?usp=drive_link" target="_blank">
                EO 15 s2023 Protocol on Technology Transfer, Intellectual Property Licensing & Commercialization
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1EnfKcCDI_SeIUtiRlcd4wZYC5vnnS4HK/view?usp=drive_link" target="_blank">
                EO 17 s2023 Policies and Guidelines on University Incentives for Literary Arts Publications
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1SPWZOXxfdfnZn72WOAuyQSZiNkuKdGQT/view?usp=drive_link" target="_blank">
                EO 18 s2023 Annex
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1uHxpmrnIbH30CUwRnsplwpLw88dAEupy/view?usp=drive_link" target="_blank">
                EO 18 s2023 Policy on Funding Support for Hosting International Research Activities
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1j6IbHeMzxEAFWUQxQhzWB5p-AzFM7c46/view?usp=drive_link" target="_blank">
                EO 19 s2023 Annex
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1zZMWlWyL4eQjdLXg3XogM0j_LuBUv8DE/view?usp=drive_link" target="_blank">
                EO 19 s2023 Poicy on Funding Support to Students for Research Production, Paper Presentationm Publication, Citation and Patent
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/19HLpmT61zhDSMj7S1FGgjiONvS0tFpGs/view?usp=drive_link" target="_blank">
                EO 21 s2023 Annex
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/17usOgDiU4Poe9d1yLf7JckCQzMP2m_fn/view?usp=drive_link" target="_blank">
                EO 21 s2023 Revised University & Externally Funded Research Policy
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/10MOM_qF_ONYyKFGuB0tJSNlwNg8jxQ8C/view?usp=drive_link" target="_blank">
                RO 22 s2022 Reconstitution of the UREDCO
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1O05EVOuACWV4zqi5XvGcf97WKGqMsLqg/view?usp=drive_link" target="_blank">
                EO 22 s2023 Appendix A
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1RkfgKZt895xZI8zHnxYhDzl2dea1P-Hc/view?usp=drive_link" target="_blank">
                EO 22 s2023 Appendix B
              </a>
            </li>
            <li class="sip">
              <a href="https://docs.google.com/document/d/1aFr4TMYl8QNUwHSf1FZnWVfAgxdJ7pjO/edit?usp=drive_link&ouid=114415632659835779612&rtpof=true&sd=true" target="_blank">
                EO 22 s2023 Appendix D
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/11AA_z57RKda-2hAaHuZv70WVM02obmx-/view?usp=drive_link" target="_blank">
                EO 22 s2023 Standards & Guidelines on Research Mentoring, Collaboration & Authorship
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1d3jekOl1Jizu85QpZi3hUhnc8tG8NlH2/view?usp=drive_link" target="_blank">
                EO 24 s2020 Implementing Guidellines on Merit Promotion
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1jg96BWeYrya9pBD8Poz7jez-mn7Ej8yn/view?usp=drive_link" target="_blank">
                EO 25 s2020 Revised UPA, Publication Citation Incentives
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1Lc4vOdVH1nNX6Ol5nTG1znMMpgSeroxk/view?usp=drive_link" target="_blank">
                EO 27 s2020 Revised Implementing Guidelines on International and National Paper Presentation Financial Assistance
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1BjLJKqc_exe0bq8bAPKc8ySZOghwvsVG/view?usp=drive_link" target="_blank">
                EO 28 s2020 Implementing Guidelines on Dissertation Financial Assistance
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1Bn3-1igXC3-YZep9A6ZCW8dtd89mtIU8/view?usp=drive_link" target="_blank">
                EO 34 s2021 Guidelines on Institute-Funded Research Project or Program
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/12WLbN2mj8p2Dgb48_apod4-ZqZULrZhh/view?usp=drive_link" target="_blank">
                EO 37 s2021 Guidelines for the Honorarium of Journal Reviewers
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1hM3JQwgVpPu24cAflsiQrkzV5Mvf4wQi/view?usp=drive_link" target="_blank">
                IPMO MO 1 s2023
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1aRPnVJiKNh1Zjhk0UBIX9hU-HcAIj3OY/view?usp=drive_link" target="_blank">
                MO 6 s2019 Guidelines for Faculty Fellowship
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/14kopfOycchpBQZBrSZWmi0z60alpfKg5/view?usp=drive_link" target="_blank">
                MO 7 s2018 University Policy Concerning Illegal Copying and Use of Unlicensed Software
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1o2U_iNzUDtjr5GgOCVBSuZ-t5DKvHSPO/view?usp=drive_link" target="_blank">
                MO 10 s2020 Revised Implementing Guidelines for Internally-Funded Researches
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1adgU6_Fx_zI_RtI2gTwaCqT8LerAADq8/view?usp=drive_link" target="_blank">
                MO 11 s2020 Revised Implementing Guidelines for Externally-Funded Researches
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1j6SCch1HJNsw2qN1g4Zu5HZfBK4kjTO6/view?usp=drive_link" target="_blank">
                MO 12 s2020 Revised Implementing Guidelines for Gawad Tuklas Outstanding Researcher Award
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1HFvpiXAwwuIR4RRcQtkzMH4vjVAAMAx9/view?usp=drive_link" target="_blank">
                MO 13 s2020 Reveised Implementing Guidelines for Thesis and Dissertation Grant
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1fsVzt8Q0ZlyvMWdKvv_lFdksq76gFjdX/view?usp=drive_link" target="_blank">
                MO 14 s2020 Reveised Implementing Guidelines of University Research Ethics Board Structures and Functions
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1vGdPDz485Dt_zyAapPLipQe5mxgFSbTy/view?usp=drive_link" target="_blank">
                MO 15 s2020 Reveised Implementing Guidelines on Incentives for Publication and Citation
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1l5UTUEEy_Sf3vcE8VfsPHqn0NuAYxLn4/view?usp=drive_link" target="_blank">
                MO 16 s2020 Revised Implementing Guidelines on International and National
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1aiMBNBEgzEdwEnI7idkV3mHRCwih4fcg/view?usp=drive_link" target="_blank">
                MO 17 s2020 Revised Implementing Guidelines on Merit Promotion on The Basis of Research Publicaiton and Citation
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1kU9kbLSzoQcrOO6rVHkjRPlpTzmCXKyK/view?usp=drive_link" target="_blank">
                MO 18 s2020 Revised Implementing Guidelines on the Utilization of College Branch Resaerch Fund
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1wPh0H1RDrMeKOrsEm_fhF8M2P7uor3eS/view?usp=drive_link" target="_blank">
                MO 20 s2019 Policy on Commercialization of Resaeerch and Development Output
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1YPjFDajSAIfehGQS4rzrNm4p6laxD8sA/view?usp=drive_link" target="_blank">
                MO 21 s2019 Revised Implementing Guidelines on Invention Disclosure Incentives
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1b-BYrGVhgZRNO3DPaHATkNbRHibETaGo/view?usp=drive_link" target="_blank">
                MO 30 s2019 Gawad Lathala International Publication Excellence Award
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1lCWfQGxSqgvUAgrlFPmkV2x5kvokjcH7/view?usp=drive_link" target="_blank">
                MO 32 s2013 Policy on Copuright Registration of Undergraduate and Gradueate Works
              </a>
            </li>
          </ul>
        </div>
      </section>
    </section>
  </section>

  <section id="development-dialog" class="exhibits-dialog-overlay">
    <section class="exhibits-dialog-box">
      <section class="header-dialog">
        <button class="close-btn" onclick="closeDevelopmentDialog()">
          <!--<img src="../assets/close.png" />-->
          󰌑
        </button>
        <div class="title-dialog">University Research and Devt. Manual</div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Research Manuals</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1X9_t2eZ3O82kCuzOTcoNi7Xx7TSyFaI6/view?usp=drive_link" target="_blank">
                University Thesis and Disertation Manual with ISBN as of 08.07.17
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1MA2373BC4rjx--dxaSRr0dxnKOoRvLSK/view?usp=drive_link" target="_blank">
                University Research Manual
              </a>
            </li>
          </ul>
        </div>
      </section>
    </section>
  </section>


  <section id="disertation-dialog" class="exhibits-dialog-overlay">
    <section class="exhibits-dialog-box">
      <section class="header-dialog">
        <button class="close-btn" onclick="closeDisertationDialog()">
          <!--<img src="../assets/close.png" />-->
          󰌑
        </button>
        <div class="title-dialog">University Thesis and Disertation</div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Social Sciences and Development Review</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1PyTke-xmtHD5C6raNNPXJKVfFC4HUIY3/view?usp=drive_link" target="_blank">
                SSDR 2021
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1sn7WFrQIxR6y2nH1RXHjITBtxN3YJKsb/view?usp=drive_link" target="_blank">
                SSDR 2020
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1JkWBK1H6TytNwHwKCsOY5g5POw954gHl/view?usp=drive_link" target="_blank">
                SSDR 2019
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1SAhJ0JpR4XXHhjvMp7L_evZaftc0QKP9/view?usp=drive_link" target="_blank">
                PUP Journal SSDR 2021
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1G0ZRg5xoVDJjrkZt2Of-9zw1tRV1u1cO/view?usp=drive_link" target="_blank">
                PUP Journal SSDR 2020
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/10pUJ7VBUG3HzRCnorXaqgqfGMHAFRkbk/view?usp=drive_link" target="_blank">
                PUP Journal SSDR 2019
              </a>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Mabini Review</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1uLo0oGQQneUubRQqcf8jqncFnwD9inxb/view?usp=drive_link" target="_blank">
                PUP Journal Mabini 2022
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1O0M9mp1VUmzEv8WBpq6OgQj5_gu0qOWO/view?usp=drive_link" target="_blank">
                PUP Journal Mabini 2021
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1mVvNnpI7gjykp1DMLSDjir-20EY2ndwW/view?usp=drive_link" target="_blank">
                PUP Journal Mabini 2019
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1f2RF3eLP37FOXP2PJBbs8-nu1i6HXz-Q/view?usp=drive_link" target="_blank">
                Mabini 2022
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1iNSXNbMefUAtWNJJcPu-h6_g-qro4JhV/view?usp=drive_link" target="_blank">
                Mabini 2021
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1iK4x11BK8VihYSJJjWpCmdSrIvHzd1S7/view?usp=drive_link" target="_blank">
                Mabini 2019
              </a>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Journal of Science and Technology</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1iBS7hLB2Wqgr95Yk59QhwG6momQNnkiI/view?usp=drive_link" target="_blank">
                S&T 2020
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/17mSG15L-TwH6gtTEOE79r8Z5NZZ_U-6I/view?usp=drive_link" target="_blank">
                S&T 2019
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1DbGd5Wfk0lxkxd4tbRxXW67rV8rqowUF/view?usp=drive_link" target="_blank">
                S&T 2018
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1yfeMFnMUf9DmZ-Yl_uIMVplIp1G6WUul/view?usp=drive_link" target="_blank">
                PUP Journal S&T 2020
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1no33okKiJ5HDIeRjWdDW2ZX8RqiGdxsw/view?usp=drive_link" target="_blank">
                PUP Journal S&T 2019
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/14GNwpfpJPzgKnX7VnmV2664QmmueG4gw/view?usp=drive_link" target="_blank">
                PUP Journal S&T 2018
              </a>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Entrada</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1S-3wYKzu8aMEQ2tV-4oZGTk_5698HdZ_/view?usp=drive_link" target="_blank">
                PUP Journal Entrada 2017
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1fdrvnT1frPhQ0EktChSQ-EYxw24caD6U/view?usp=drive_link" target="_blank">
                PUP Journal Eentrada 2015 2(1)
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1Yaf0uISNI0JzaePKUtZffhsLn9KEhA6Q/view?usp=drive_link" target="_blank">
                PUP Journal Eentrada 2015 2(2)
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1zO1i4GZxJpWMDT79YPGn1-GYxSvxgmo0/view?usp=drive_link" target="_blank">
                Entrada 2017
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/15NFLhbWQIaxS8h49-06aa4kHZ8hum5SC/view?usp=drive_link" target="_blank">
                Entrada 2015 2(1)
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1a-AijEIgU3BbD7076yle0t_z0y1eC8Oq/view?usp=drive_link" target="_blank">
                Entrada 2015 2(2)
              </a>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Education Review</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1PzZdzFnQSSXMS2ut1q5wxAGb0w0XvJfK/view?usp=drive_link" target="_blank">
                EducReview 2021
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1knISLOcfxdbyskxVaK8WY0z1XmrfIfnS/view?usp=drive_link" target="_blank">
                EducReview 2015
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1oa-WcQ9LjCQ7Piq8UmIwo65GDIKzUfju/view?usp=drive_link" target="_blank">
                EducReview 2022
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1-lRPHiGD1ukdTMWnDeTKNY7j73D18HaZ/view?usp=drive_link" target="_blank">
                PUP Journal Education Review 2021
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1PWO49RxB4jf5ySVHvSZi4ROYhFRY2oqy/view?usp=drive_link" target="_blank">
                PUP Journal Education Review 2015
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1q9K9UrLCIcl0JjhXgjZFbo-Z8s5iJ-Hw/view?usp=drive_link" target="_blank">
                PUP Journal Education Review 2022
              </a>
            </li>
          </ul>
        </div>
      </section>
      <section class="collapsible-container">
        <button class="collapsible">
          <div class="title">Bisig</div>
        </button>
        <div class="content">
          <ul>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1Ooz7Jj1LDGr04nZ86t4eFTgynVAGrjIf/view?usp=drive_link" target="_blank">
                PUP Journal Bisig 2014
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1hFBtoBlupl5lkziKXI9nXIeLNuf_j-3y/view?usp=drive_link" target="_blank">
                PUP Journal Bisig 2015
              </a>
            </li>
            <li class="sip">
              <a href="https://drive.google.com/file/d/1URSvWqaBRkkZbG6DkyP5R53L_WoifPVj/view?usp=drive_link" target="_blank">
                PUP Journal Bisig 2021
              </a>
            </li>
          </ul>
        </div>
      </section>
    </section>
  </section>

  <?php include('../assets/static-html/footer.html'); ?>

  <script src="../main-script.js"></script>
</body>

</html>
